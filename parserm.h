...
!
!   Diese Datei wird automatisch von "Parser" eingebunden.
! ------------------------------------------------------------------------------
...
Constant STUCK_PE     = 1;          ! Fehler mit höheren Nummer sind wichtiger,
Constant ANIMA_PE     = 2;          ! bzw. aussagekräftiger. Es wird versucht,
Constant UPTO_PE      = 3;          ! aussagekräftigsten Fehler auszugeben.
Constant NUMBER_PE    = 4;
Constant CANTSEE_PE   = 5;          ! ANIMA_PE, eh ein unglücklicher Fehler, ist
Constant TOOLIT_PE    = 6;          ! wenig aussagekräftig und könnte genauso
Constant NOTHELD_PE   = 7;          ! gut in der Aktion selbst abgefangen
Constant MULTI_PE     = 8;          ! werden.
Constant MMULTI_PE    = 9;
Constant VAGUE_PE     = 10;         ! Einer Anregung von Olav Müller folgend
Constant EXCEPT_PE    = 11;         ! (Danke!), habe ich ihn von 11 auf zwei
Constant VERB_PE      = 12;         ! zurückgestuft, da es oft Probleme mit
Constant SCENERY_PE   = 13;         ! ganzen Verben gab, solange sie als erstes
Constant ITGONE_PE    = 14;         ! Token [creature] hatten. Dieses Problem
Constant JUNKAFTER_PE = 15;         ! tritt auch in der Original-Lib bei
Constant TOOFEW_PE    = 16;         ! Give reverse auf.
...
Include "German";               ! Hier wird nicht language__, sondern direkt
                                ! German.h eingebunden, da auch in den anderen
                                ! Dateien Änderungen gemacht werden müssen.
                                ! Somit entfällt auch +language_name=German
...
[ Parser__parse  results   syntax line num_lines line_address i j k
  ...
  .ReParse;
    ...
    max_wn = -1; ! hierher vorverlegt von Abschnitt E
  ...
  .BeginCommand;
    ...
    if (is_again_word(verb_word) == false)
            ...
            if (line_ttype-->pcount == ELEMENTARY_TT) {
                ...
                if (line_tdata-->pcount == MULTIEXCEPT_TOKEN or MULTIINSIDE_TOKEN  && i == 1) {
                    ...
                    if (line_ttype-->pcount == PREPOSITION_TT) {
                        while (Lookahead_Skip(line_ttype-->pcount, line_tdata-->pcount)) pcount++;

                        if (Lookahead_Valid(line_ttype-->pcount, line_tdata-->pcount)) {
                            ...
                            while (wn < num_words) {
                                ...
                                if ( l && (l->#dict_par1) &8 ) {   ! if preposition
                                    ...
                                    l = Lookahead_Parse(line_ttype-->pcount, line_tdata-->pcount);
                                    #Ifdef DEBUG;
                                    if (parser_trace >= 2) {
    ...
    for (flag=true : flag :) {
       ...
       for (x=1 : x<=LanguageDescriptors-->0 : x=x+4)
        ...
        if (is_all_word(o)) {
            ...
            indef_mode = 1; flag = 2; indef_wanted = 100;
...
[ ParseToken__ given_ttype given_tdata token_n
        ...
        switch (given_tdata) {
          ...
          TOPIC_TOKEN:
            ...
            if (action_to_be == ##Ask or ##Answer or ##Tell) {
                ...
                ! Das erste Wort wird als "Ergebnis" übergeben. Damit dies
                ! kein Artikel ist, werden diese hier mit einer sehr kruden
                ! Mechanik übersprungen.
  ...
  .NextInList;
    ...
    if (is_and_word(o) || is_but_word(o) || o == comma_word) {
    ...
    if (indef_mode == 1 && indef_type & PLURAL_BIT ~= 0) {
        ...
        for (j=BestGuess() : j~=-1 && i<indef_wanted && i+offset<63 : j=BestGuess()) {
            ...
            if (action_to_be == ##Take or ##Remove && parent(j) == actor)
                flag = 0;
            k = LanguageChooseObjects(j, flag);
            if (k == 1)
                flag = 1;
            else {
                if (k == 2) flag = 0;
...
!   if (indef_type & OTHER_BIT ~= 0) threshold++;
    ...
    for (i=0 : i<number_matched : i++) {
        ...
        if (met < threshold) {
            ...
            its_score = its_score + SCORE__CHOOSEOBJ * LanguageChooseObjects(obj, 2);
...
! ----------------------------------------------------------------------------
!  LanguageChooseObjects deklariert einige einfache Regeln zur Bewertung von
!  Objekten beim Disabiguisieren und für "nimm alles".  Ob noun oder second
!  betrachet wird, gibt (annäherungsweise) die Variable parameters an, die die
!  bereits geparsten Ergebnistokens (noun, held usw.) anzeigt. Sie ist null
!  für noun und eins für second. (Und größer für Objektlisten.)
! ----------------------------------------------------------------------------

[ Disambiguate obj    o;
    switch (action_to_be) {
        ##Take, ##Remove:
            if (obj has scenery or static or animate) return 2;
        ##Drop:
            if (obj has scenery or static) return 2;
        ##PutOn:
            if (parameters == 0) {
                if (obj has scenery or static) return 2;
            }
            if (parameters == 1 && obj has supporter) return 5;
        ##Insert:
            if (parameters == 0) {
                if (obj has scenery or static) return 2;
            }
            if (parameters == 1 && obj has container) {
                if (obj has open) return 5; return 4;
            }
        ##Enter:
            if (obj has enterable) return 5;
        ##Go:
            if (obj == out_obj && real_location provides out_to) return 5;
        ##Search:
        ##Unlock, ##Lock:
            if (parameters == 0 && obj has lockable) {
                if (obj has locked && action_to_be == ##Unlock) return 5;
                if (obj hasnt locked && action_to_be == ##Lock) return 5;
                return 4;
            }
            if (parameters == 1) {
                objectloop (o provides with_key) {
                    if (o.with_key == obj) return 5;
                }
            }
        ##SwitchOn, ##SwitchOff:
            if (obj has switchable) {
                if (obj has on && action_to_be == ##SwitchOff) return 5;
                if (obj hasnt on && action_to_be == ##SwitchOn) return 5;
                return 4;
            }
        ##Open, ##Close:
            if (obj has openable) {
                if (obj has open && action_to_be == ##Close) return 5;
                if (obj hasnt open && action_to_be == ##Open) return 5;
                return 4;
            }
        ##Disrobe, ##Wear:
            if (obj has worn) return 5;
            if (obj has clothing) return 4;
        ##Eat:
            if (obj has edible) return 5;
    }
    if (obj has scenery or concealed) return 2;
    return 3;   ! Der Default ist nicht Null, um Objekte niedriger als
                ! normal priorisieren zu können.
];

[ DisambiguateAll obj;
    if (obj has concealed || obj has scenery) return 2;
    if (action_to_be == ##Take or ##Remove) {
        if (obj has static or animate) return 2;
    }
    ! Die verbleibenden Multi-Verben sind ##Drop, ##PutOn, ##Insert
    ! zu denen mir keine sinnvolle Einschränkung einfällt.

    rfalse;
];

[ LanguageChooseObjects obj flag    a k;
    if (flag == 2) {
        ! Hier werden einige Objekte bevorzugt, andere benachteiligt
        a = action; action = action_to_be;
        k = RunRoutines(obj, disambig);
        action = a;
        #ifdef DEBUG;
        ! Meckern, wenn zu hoch priorisiert wird
        if (k < -3 || k > 10) print "^[*** deform: Disambig sollte Werte
            von -3 bis 10 zurückgeben, aber ", (withoutArt) obj, " gibt ",
            k, " zurück. ***]^";
        #endif;

        if (k) return k + 3;
        #ifdef TRADITIONAL_CHOOSE_OBJECTS;
        return ChooseObjects(obj, flag);
        #ifnot;
        k = ChooseObjects(obj, flag);
        if (k) return k;
        return Disambiguate(obj);
        #endif;
    } else {
        ! Hier wird geschaut, was in "nimm alles" berücksichtigt werden soll
        #ifdef TRADITIONAL_CHOOSE_OBJECTS;
        return ChooseObjects(obj, flag);
        #ifnot;
        k = ChooseObjects(obj, flag);
        if (k) return k;
        return DisambiguateAll(obj);
        #endif;
    }
];
...
[ PrintCommand from i k spacing_flag prep_before dative_flag;
    ...
    for ( : k<pcount : k++) {
        ...
        if (i == PATTERN_NULL) {
            ...
            if (line_ttype-->(k-1) == GPR_TT && line_tdata-->(k-1) == dative)
                dative_flag = true;
            continue;
        }
        ...
        if (i >= REPARSE_CODE) {
            print (UmlautAddress) No__Dword(i-REPARSE_CODE);
            spacing_flag = true;
            prep_before = true;
            continue;
        } else {
            if (i in compass && LanguageVerbLikesAdverb(verb_word)) {
                print "nach ";    ! etwas wie "nach Osten"
...
[ SearchScope domain1 domain2 context i;
    ...
    dative_mode = false;     ! Hier zurücksetzen, weil - so glaube ich -
                             ! hier parse_name und die normale name-Suche
                             ! für alle Objekte abgeschlossen sind.
];
...
[ Refers obj wnum   wd k m;
    ...
    ! k = wn; wn = wnum;
    ! m = StringInProperty(obj, long_name);
    ! wn = k;
    ! return m;
];

[ IsGenderAttribute m;
    if (m == male or female or neuter or pluralname) return m;
    if (m == MALE_GENDER) return male;
    if (m == FEMALE_GENDER) return female;
    if (m == NEUTER_GENDER) return neuter;
    if (m == PLURAL_GENDER) return pluralname;
    rfalse;
];
...
[ WordInProperty wd obj prop k l m;
    ...
    for (m = 0 : m < l : m++)
        if (wd == k-->m) {
            if (m < l-1 && (k-->(m+1) > 255))
                GenderNotice(obj, IsGenderAttribute(k-->(m+1)));
            rtrue;
        }
    rfalse;
];

[ StringInProperty obj prop k l m;
    ! Prüft analog zu WordInProperty, ob das Wort an der Stelle wn in
    ! der Eigenschaft prop abgelegt ist. Verwendet zur Überprüfung
    ! WordMatch(s, false).

    k = obj.&prop; l = (obj.#prop)/WORDSIZE;
    print (object) obj, ": ", wn, ", ", (address) parse-->(2*wn - 1) , " ^";
    wn--;
    for (m = 0 : m < l : m++)
        if (WordMatch(k-->m)) {
            wn--;
...
[ WordMatch s exact reverse    sl wl sa wa;
    ...
    #ifnot;
    sl = PrintAnyToArray(StorageForShortName, 40, s);
...
! ----------------------------------------------------------------------------
!   Die folgenden Routinen implementieren den prep- und parse_ref-Mechanismus,
!   wenn EXTENDED_PARSER definiert ist.
!
!   Statt parseNoun muss daher aus den internen Parser-Routinen DoParseNoun
!   aufgerufen werden.
! ----------------------------------------------------------------------------

#ifdef EXTENDED_PARSER;

Global block_parsing = false;

[ IsDescriptor wd
    n;

    for (n = 1 : n <= LanguageDescriptors-->0 : n = n + 4) {
        if (wd == LanguageDescriptors-->n) rtrue;
    }
    rfalse;
];
...
[ WordAt wn;
#ifdef TARGET_ZCODE;
    if (wn > parse->1) rfalse;
    return parse-->(wn * 2 - 1);
#ifnot;
    if (wn > parse-->0) rfalse;
    return parse-->(wn * 3 - 2);
#endif;
];

!   Diese Routine macht genau das, was im DM4, §28 beschrieben wird,
!   Im Gegensatz zum Parser selbst, bei dem in TryGivenObject(obj)
!   auch 1 zurückgegeben wird, wenn es keinen Treffer gab.

[ DoParseObject obj block
    n own oblock desc;

    oblock = block_parsing;
    block_parsing = block;

    ! Descriptors, quick and dirty
    while (IsDescriptor(WordAt(wn))) {
        wn++;
        desc++;
    }
    own = wn;

    ! parse_name überprüfen
    if (obj provides parse_name) {
        n = RunRoutines(obj, parse_name);
        if (n == 0) return 0;
        if (n > 0) {
            wn = own + desc + n;
            block_parsing = oblock;
            return desc + n;
        }
    }
    wn = own;
    ...
    ! Wie gehabt: Longest match in name
    n = 0;
    wn = own;
    while (WordInProperty(NextWord(), obj, name)) n++;
    wn = own + desc + n;
    block_parsing = oblock;
...
!   Dieser ParseNoun-Einhänger berücksichtigt Objekte, die analog
!   zum Namen eine Eigenschaft prep (wie preposition) haben.
!
!   Wenn das Objekt die Eigenschaft parse_ref definiert, sollte
!   diese auf ein Objekt verweisen, dessen Name nach den in prep
!   angegebenen Präpositionen untersucht wird. Dieses Objekt kann
!   auch selbst wieder mit parse_name oder ParseNoun untersucht
!   werden.
!
!   Der einfachere Fall ist, wenn es kein parse_ref gibt: Dann
!   werden alle Wörter in name und prep sowie alle Descriptors,
!   also eigentlich nur alle Artikel verstanden. Die Wortkette
!   darf aber nur mit einem Wort aus name aufhören, damit zum
!   Beispiel "nimm die Karte aus der Schweiz aus dem Postkorb"
!   richtig interpretiert wird.

[ DoParseNoun obj
    n m nn wd ref own oown prefix;

    ! Einhänger, eventuell vom Autor definiert
    m = ParseNoun();
    if (m >= 0) return m;
    m = 0;

    if (~~(obj provides prep)) return -1;

    if (obj provides parse_ref) {

        if (block_parsing == obj) return -1;
        ...
        ! Nachgestelltes Objekt parsen, falls der Parser hier
        ! Wörter zur Disambiguisierung vorangestellt hat:
        ! "auf Holzfigur Fleck auf Figur"

        while (WordInProperty(NextWord(), obj, prep)) nn++;
        wn = own + nn;
        prefix = DoParseObject(ref, 0);

        if (nn > 0 && prefix == 0) return -1;
        prefix = prefix + nn;
        wn = own + prefix;
        nn = 0;
        ...
        ! Hauptobjekt parsen ("weißer Fleck")
        n = DoParseObject(obj, obj);
        if (n < 1) return 0;

        ! Eventuell nachgestellte Präpositionen parsen
        wn = oown + n;
        while (WordInProperty(NextWord(), obj, prep)) m++;
        if (m == 0) return prefix + n;

        ! Bezugsobjekt parsen ("Holzfigur")
        wn = oown + n + m;
        nn = DoParseObject(ref, 0);
        if (nn) return prefix + m + n + nn;
        return prefix + n;
    }

    wd = NextWord();
    while (WordInProperty(wd, obj, name)
        || WordInProperty(wd, obj, prep)
        || IsDescriptor(wd)) {
        n++;
        if (WordInProperty(wd, obj, name)) m = n;
        wd = NextWord();
    }
    return m;
];

#ifnot; ! EXTENDED_PARSER

[ DoParseNoun obj;
    return ParseNoun(obj);
];

#endif; ! EXTENDED_PARSER
...
[ PronounsSub x y c d;
    ...
    if (player ~= selfobj) {
        print "~mich~/~mir~ "; L__M(##Pronouns, 2);
            ...
            while (~~deadflag) {    ! everything happens in this loop
              ...
              .begin__action;
                ...
                for (k=1 : k<=j : k++) {
                    ...
                    ! Hier wird in der deutschen Lib nicht (name), sondern
                    ! die Routine (WithoutArt) verwendet. Sie setzt die Endungen
                    ! korrekt und berücksichtigt auch adj und post.
...
! ------------------------------------------------------------------------------
!   Die Routinen zur Ausgabe von Objektnamen stehen in German.h
! ------------------------------------------------------------------------------
...
Constant DEFORM___2010_03_16___PARSERM;                ! for dependency checking
Constant DEFORM___PARSERM;

...
!
!   Diese Datei wird automatisch von "VerbLib" eingebunden.
! ==============================================================================
...
Constant APPEND_BIT     8192;       ! Hängt die Inhalte der gelisteten Objekte
                                    ! an, anstatt sie in einem Nebensatz zu
                                    ! erwähnen

Default LIST_BUFFER_SIZE = 20;
...
[ WriteSublists o n;
    ...
    while (list_buffer-->0) {
        ...
        if (list_depth-->(list_buffer-->0)) print " "; else print "^^";
        (list_buffer-->0)--;
        L__M(##Look, -1, o);
        n++;
    }
...
[ WriteListR o depth stack_pointer
             classes_p sizes_p i j k k2 l m n q senc mr;
    ...
    for (i=1,j=o,k=0,mr=0 : senc>=0 : i++,senc--) {
        ...
        if (j.list_together ~= 0 or lt_value) {
            ...
            if (k2 == 2 or 3) {
                ...
                if (c_style & INDENT_BIT ~= 0) {
                    Print__Spaces(INVENTORY_INDENT*(depth+wlf_indent));
      ...
      .Omit_WL2;
        ...
        if (sizes_p->i == 1) {
            if (c_style & NOARTICLE_BIT ~= 0) WithoutArt(j, short_name_case);
            else {
                if (c_style & DEFART_BIT ~= 0) DefArt(j, short_name_case);
                else IndefArt(j, short_name_case);
            }
        }
        else {
...
[ PushListBuffer o depth;
    if (list_buffer-->0 == LIST_BUFFER_SIZE) rfalse;
    (list_buffer-->0)++;
    list_buffer-->(list_buffer-->0) = o;
...
[ WriteAfterEntry o depth stack_p
    ...
    if (child_count && (c_style & ALWAYS_BIT) && (c_style & APPEND_BIT)==0) {
...
[ InvSub x;
    ...
    if (inventory_style & NEWLINE_BIT ~= 0) {
        L__M(##Inv, 3);
        x = 1;          ! Aufzählungslisten fangen auf Ebene 1, nicht 0 an
    } else print " ";

    WriteListFrom(child(player), inventory_style, x);
    if (inventory_style & ENGLISH_BIT ~= 0) L__M(##Inv, 4);
    if (inventory_style & ENGLISH_BIT ~= 0 && WriteSublists()==0) print "^";
...
[ AttemptToTakeObject item     ancestor after_recipient i j k a s;
        ...
        if (SACK_OBJECT ~= 0) {
            ...
            objectloop (k in player)
                if (k ~= SACK_OBJECT && k hasnt worn && k hasnt light) {
                    a = action; s = second;
                    action = ##Insert; second = SACK_OBJECT;
                    i = RunRoutines(k, before_implicit);
                    action = a; second = s;
                    if (i == 0) j = k;
                }
...
[ RemoveSub i;
    i = parent(noun);
    if (i has container && i hasnt open) return L__M(##Remove, 1, second);
...
!   Auch der Locale muss der passende Fall für die Liste übergeben werden.
...
[ DisrobeSub;
    if (ObjectIsUntouchable(noun)) return;
    if (noun notin player || noun hasnt worn) return L__M(##Disrobe, 1, noun);
    give noun ~worn;
    if (AfterRoutines() == 1) rtrue;
    if (keep_silent == 1) rtrue;
    L__M(##Disrobe,2,noun);
...
! ----------------------------------------------------------------------------
!   Verben, die in deform definiert sind
! ----------------------------------------------------------------------------

[ DressSub;
    if (noun==player) return L__M(##Dress, 2);
    L__M(##Dress, 1, noun);
];

[UndressSub;
    if (noun==player) return L__M(##Undress, 2);
    L__M(##Undress, 1, noun);
];

[ LiftSub;
    if (noun has static) return L__M(##Lift, 1, noun);
    if (noun has scenery) return L__M(##Lift, 2, noun);
    if (noun has animate) return L__M(##Lift, 3, noun);
    <<LookUnder noun>>;
];
...
[ EchoOnSub; parser_echo = 1; "[Echo on.]"; ];

[ EchoOffSub; parser_echo = 0; "[Echo off.]"; ];
...
[ XObj obj f;
    ...
    if (obj == Class)
        WriteListFrom(child(obj),
            NEWLINE_BIT + INDENT_BIT + ALWAYS_BIT + NOARTICLE_BIT, 1);
    else
        WriteListFrom(child(obj),
            NEWLINE_BIT + INDENT_BIT + ALWAYS_BIT + FULLINV_BIT, 1);
];
...
! ----------------------------------------------------------------------------
!   Hier werden die Objekte kurz überprüft.
! ----------------------------------------------------------------------------

[ isRoom o;
    if (o provides cant_go) rtrue;
    if (o provides n_to or s_to or w_to or e_to or
        sw_to or se_to or ne_to or nw_to or
        u_to or d_to or in_to or out_to) rtrue;
    rfalse;
];
...
[ UmlautCheck i letter length start invalid;
    ...
    #ifdef TARGET_GLULX;
    length = PrintAnyToArray(StorageForShortName, 24, i);
    start = 0;
    #ifnot;
...
[ XLibCheckSub    o n errors;
    objectloop (o ofclass Object) {
        if (o hasnt proper && (o provides name or parse_name)
            && isRoom(o)==false) {
            n = 0;
            if (o has male) n++;
            if (o has female) n++;
            ...
            if (n ~= 1) {
                ...
                if (n) print "keine eindeutige Genus-Definition.^";
                else print "keine Angabe zum Genus.^";
                errors++;
            }
        }
        for (n = 0 : n<o.#name/WORDSIZE : n++) {
            if (UmlautCheck(o.&name-->n)) {
                ...
                "' von Objekt ", o, " (", (der) o, ") hat Umlaute.^";
                errors++;
            }
        }
    }
...
Constant DEFORM___2010_03_16___VERBLIBM;               ! for dependency checking
Constant DEFORM___VERBLIBM;